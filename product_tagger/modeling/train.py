...
def build_dataloaders(
    ...
    - Lee el CSV de entrenamiento para inferir el conjunto de clases y crear
      un mapeo estable clase -> índice entero (`class_to_idx`) si no se
      proporciona uno.
    - Aplica opcionalmente data augmentation en el conjunto de entrenamiento
      manteniendo un pipeline de normalización coherente.
    """
...
def main(
    ...
    Puede aplicar data augmentation en el conjunto de entrenamiento mediante
    transformaciones de `torchvision` (flip, color jitter, rotación) cuando
    `use_augmentation=True`, y aplica early stopping controlado por `patience`.
    """
    ...
    # DataLoaders con o sin augmentation según parámetro
    ...
    run_name = "vit_articleType_aug" if use_augmentation else "vit_articleType_no_aug"
    augmentation_desc = (
        "torchvision_flip_colorjitter_rotate" if use_augmentation else "none"
    )
        ...
        mlflow.log_params(
            {
                ...
                "num_workers": num_workers,
                "label_col": label_col,
                "augmentation": augmentation_desc,
